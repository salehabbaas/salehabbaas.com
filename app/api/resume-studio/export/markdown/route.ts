...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const exported = await exportDocument({ format: "markdown", doc, fidelityMode: "semantic" });
    const text = typeof exported.body === "string" ? exported.body : exported.body.toString("utf-8");
    ...
    const message = error instanceof Error ? error.message : "Unable to export Markdown";
