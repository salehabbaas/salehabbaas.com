...
import { priorityToneMap, type DashboardKpis, type ImportantTaskRow, type ProjectDoc, type ProjectMetrics } from "@/types/project-management";
...
export function ProjectDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error ?? "Unable to load project dashboard");
      setPayload(data);
    } catch (error) {
      ...
      <div className="grid gap-4 xl:grid-cols-[1.5fr_1fr]">
              ...
              return (
                <Link key={project.id} href={`/admin/projects/${project.id}`} className="rounded-2xl border border-border/70 bg-card/80 p-4 hover:border-primary/45">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                    ...
                    <p className="rounded-lg border border-border/70 px-2 py-1">P1: {metrics?.p1Count ?? 0}</p>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Sorted by overdue, P1, and nearest deadline.</CardDescription>
          </CardHeader>
            ...
            {importantTasks.map((entry) => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge className={priorityToneMap[entry.task.priority]}>{entry.task.priority}</Badge>
                </div>
