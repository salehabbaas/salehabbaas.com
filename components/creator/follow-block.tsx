...
export function FollowBlock({ socialLinks }: Pick<CreatorSettings, "socialLinks">) {
  return (
    <div className="rounded-3xl border border-border/70 bg-gradient-to-br from-white to-cyan-50 p-6">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {socialLinks.map((link) => (
          <Link
            ...
            onClick={() => trackEvent("click_social", { platform: link.label.toLowerCase() })}
          >
            {link.label}
          </Link>
        ))}
