...
> Review note (2026-03-17): This file has been re-audited against the current codebase after the recent Resume Studio viewer/editor work. Status labels use `Implemented`, `Partially implemented`, and `Not implemented`.

## Goal

> Status: Partially implemented. Notes: Resume Studio already supports admin-only access, real-time editing, ATS checks, AI rewriting, versioning, and PDF/HTML/TXT export. The planned schema-first editor rebuild, Markdown/DOCX export, and Google-Docs-level authoring model are not in place.
...
> Status: Partially implemented. Notes: The diagnosis of current editor fragility is still accurate. The recommended replacement of the contentEditable-centered model with a schema-first editor engine has not been implemented.
...
> Status: Not implemented. Notes: The codebase still uses the existing custom contentEditable editor and has not adopted Tiptap/ProseMirror.

## Use This Stack

> Status: Not implemented. Notes: No Tiptap/ProseMirror editor module exists in the current repository.
...
> Status: Partially implemented. Notes: The domain layer is strong and production-ready. The structured content layer described here is not implemented, while the render/export layer exists in a lighter form based on the current section model.

## Target End State

> Status: Partially implemented. Notes: Layer 1 (domain) is implemented. Layer 2 (structured content JSON as canonical editor source) is not implemented. Layer 3 (render/export transforms) is partially implemented for print, PDF, ATS, and TXT.
...
> Status: Partially implemented. Notes: The product already behaves like a resume-specific system with template and ATS guardrails, but it does not yet enforce the richer structured editing constraints proposed here.
...
> Status: Not implemented. Notes: Resume sections still store ad hoc `data` objects and some rich text is still handled through sanitized HTML/contentEditable rather than a typed rich document AST.
...
> Status: Partially implemented. Notes: `schemaVersion` exists and Resume Studio is normalized to schema version 2, but there is no `editorModelVersion`, `editorEngine`, or `contentFormat` metadata.
...
> Status: Implemented. Notes: The current system is strongly section-based with typed section kinds, template region mapping, ATS logic, print rendering, and section-level editing.
...
> Status: Partially implemented. Notes: A local `editor-v2` foundation now exists under the Resume Studio component/lib stack, including rich-text editor primitives, content helpers, and migration helpers. It is not yet a full package matching the architecture proposed here.
...
> Status: Partially implemented. Notes: The product has a resume-specific section model and template schema, but not the proposed node/mark-based structured editor schema with rich document blocks and formatting marks.
...
> Status: Partially implemented. Notes: The editor has a selection toolbar and editor actions, but there is no centralized command bus or shared command layer across top toolbar, floating toolbar, shortcuts, and context menus.
...
> Status: Partially implemented. Notes: There is an A4 canvas editor, print view, zoom, page margins, template layout regions, and on-demand PDF preview/export. Missing pieces include true headers/footers, page numbers, widow/orphan rules, and a separate editor-vs-print layout engine.
...
> Status: Implemented. Notes: Templates already define `styleTokens` for fonts, sizes, colors, spacing, border radius, icon style, layout grid, and constraints, and the template builder edits these directly.
...
> Status: Partially implemented. Notes: The system already applies ATS warnings, margin normalization, two-column ATS penalties, template constraints, and template-specific defaults. It does not yet enforce the full set of font-size, table, image, and overflow guardrails proposed here.
...
> Status: Partially implemented. Notes: Resume import from PDF/TXT/MD and PDF template extraction are implemented with parsing, normalization, and AI structuring. General HTML/Markdown/DOCX rich-document import into a structured editor schema is not implemented.
...
> Status: Partially implemented. Notes: The current editor sanitizes rich HTML with explicit allowed tags and styles, but the full import-time policy described here is not implemented across all planned formats.
...
> Status: Partially implemented. Notes: The current rich editable fields intercept paste and insert plain text for safety. Multiple paste modes such as Office-aware paste, paste without formatting shortcut handling, and full semantic normalization are not implemented.
...
> Status: Partially implemented. Notes: Undo/redo, drag-and-drop, keyboard selection formatting, search/replace, word count, character count, and outline extraction now exist in the editor stack. TOC generation, page navigation, broader paste modes, and a full unified productivity surface are still not implemented.
...
> Status: Not implemented. Notes: There is no Markdown editor/export pipeline. `.md` files can be imported as plain text through resume import, which does not satisfy this planned feature.
...
> Status: Partially implemented. Notes: Documents include a language mode/value field, and AI routes accept language hints. Block/document direction controls, explicit RTL/LTR editing controls, and bilingual layout support are not implemented.
...
> Status: Partially implemented. Notes: Manual version snapshots and basic version history are implemented. Comments, anchored threads, suggestions, diff review UI, real-time collaboration, cursors, and presence are not implemented.
...
> Status: Partially implemented. Notes: Export already avoids direct DOM scraping and renders through print/PDF/text logic based on the document model. HTML print, PDF, and TXT exist; Markdown and DOCX export do not.
...
> Status: Not implemented. Notes: DOCX import/export is not present in the current codebase.
...
> Status: Implemented. Notes: ATS and quality flows already flatten document content into normalized plain text, preserve section/bullet semantics, and use dedicated serializers for scoring and export-safe text output.
...
> Status: Partially implemented. Notes: There is a `resume-studio-v2` migration and schema normalization for current document/template records. The planned dual-content migration (`contentHtmlLegacy` plus `contentDoc`) and legacy HTML-to-structured-document upgrade path are not implemented.
...
> Status: Partially implemented. Notes: Admin auth, owner checks, feature flags, sanitization, URL validation, import path checks, and file type limits are implemented in several paths. A unified structured-content import security layer and broader media/embed policy are not implemented.
...
> Status: Partially implemented. Notes: Truthfulness checks already validate AI-improved text, tailored resumes, and cover letters against source resume/job facts. Diff output exists for AI improve responses, but AI provenance tracking and structured-node-level review/apply controls are not implemented.
...
> Status: Partially implemented. Notes: The current product covers part of this list: basic formatting, links, images in plan only not current resume output, tables not as an editor feature, undo/redo, drag/drop, sanitization, page margins, and PDF/HTML/TXT exports. Search/replace, word count, Markdown/DOCX import-export, page numbers, headers/footers, and many paragraph/layout controls are not implemented.
...
> Status: Partially implemented. Notes: Version history exists. Comments, suggestions/track changes, diff view UI, outline, TOC, checklist, bookmarks, footnotes, and advanced table controls are not implemented. Template-level layout editing is implemented in the current template builder.
- Comments
- Suggestions / track changes
- Version diff view
- Outline view
...
> Status: Not implemented. Notes: Multi-user editing, user cursors, document-body mentions/tags, embedded files, and video support are not present.
- Multi-user editing
- User cursors
- Video
...
> Status: Partially implemented. Notes: The product already has a floating selection toolbar, side rail, ATS panels, preview/export panel, history panel, and template/design panels. It does not yet have the full top-toolbar + left navigator + right inspector architecture proposed here.
...
> Status: Partially implemented. Notes: The current Firestore model already stores templates, versions, exports, ATS metadata, linked jobs, and style/page settings. The proposed v2 persistence model with `status`, page headers/footers, page numbers, language direction defaults, and section-level structured docs is not implemented.
...
> Status: Partially implemented. Notes: Runtime feature flags already exist and the current module is deployable/QA-documented. The staged `editor-v2` rollout plan has not started.

## Stage 0 — Preparation

> Status: Partially implemented. Notes: General Resume Studio feature flags exist and a dedicated `resumeEditorV2Enabled` rollout flag is now present. Migration fixtures and the new persistence contract for the full structured editor model are still not implemented.
...
> Status: Partially implemented. Notes: The current production editor is still the original custom implementation, but `editor-v2` core pieces now exist: rich-text content helpers, search/replace, outline extraction, migration comparison helpers, and shared toolbar/alignment support.
...
> Status: Partially implemented. Notes: Import/sanitize exists for current resume/template flows, but not for a structured editor-v2 migration path.
...
> Status: Partially implemented. Notes: Print rendering and PDF export are in production now, but not through the proposed editor-v2 structured pipeline.
...
> Status: Implemented for the current editor. Notes: ATS, quality scan, AI improve, cover-letter generation, and job tailoring are already integrated with the existing document model.
...
> Status: Partially implemented. Notes: Runtime flags exist for Resume Studio features in general, but there is no dedicated rollout of a new editor version.
...
> Status: Not implemented. Notes: No editor-v2 full migration has happened.
- bulk migrate
- keep rollback path
...
> Status: Still relevant. Notes: Most of these risks remain valid because the recommended rebuild has not yet been executed.
...
> Status: Partially met. Notes: ATS reliability, export, version history, and template consistency are reasonably strong in the current system. Stable advanced formatting, structured migration safety, and future-ready collaboration architecture are not yet achieved.
...
> Status: Not implemented. Notes: This remains a build prompt/spec rather than a reflection of the current implementation.
...
> Status: Not started as written. Notes: Some later capabilities exist in the current product, but the sequence for the proposed `editor-v2` rebuild has not been executed.
...
> Status: Not achieved as a full v2 program. Notes: Pieces of Milestones 3-4 exist in the current system through the legacy editor and export stack, but the milestone set for the structured `editor-v2` rebuild is not complete.
...
> Status: Still accurate. Notes: The current codebase confirms this recommendation: Resume Studio is feature-rich, but the advanced editor foundation is still the old custom contentEditable-based system rather than the schema-first rebuild proposed here.
